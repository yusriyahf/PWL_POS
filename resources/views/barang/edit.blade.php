    ...
    <div class="card-body">
        ...
        @else
            <form method="POST" action="{{ url('/barang/'.$barang->barang_id) }}" class="form-horizontal" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    <label for="berkas" class="col-1 control-label col-form-label">Gambar Profile</label>
                    <div class="col-11">
                        <input type="file" class="form-control" id="berkas" name="berkas" value="{{ old('image', $barang->image) }}" required>
                        @error('berkas')
                            <small class="form-text text-danger">{{ $message }}</small>
                        @enderror
                    </div>
